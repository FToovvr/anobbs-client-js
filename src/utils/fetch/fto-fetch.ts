...
import nodeFetch, { RequestInit as NodeFetchRequestInit, Headers, Response as NodeFetchResponse } from 'node-fetch';
...
export default function makeFtoFetch<FetchRequestInit extends NodeFetchRequestInit>(
    _fetch?: (info: string, init?: FetchRequestInit) => Promise<NodeFetchResponse>,
): (info: string, init?: FetchRequestInit & RequestInitExPart) => Promise<NodeFetchResponse & WithRequest> {
    const fetch = _fetch ?? nodeFetch;

    const ftoFetch = (async (input: string, init?: FetchRequestInit & RequestInitExPart): Promise<Response> => {
        ...
        const { input: finalInput, init: finalInit, local } = await (async (input, init)=>{
            ...
            return {
                input, init: init as NodeFetchRequestInit,
                local: {
                    jar, jarOptions,
                    timeout,
                    validateStatus,
        ...
        for (let currentAttempts = 1; ; currentAttempts++) {
            ...
            try {
                // @ts-expect-error TODO: 之后清理
                const _response = await fetch(finalInput, { ...finalInit, redirect: 'manual' });
